...
(() => {
  ...
  function showReport(data) {
    ...
    el("report-summary").textContent = `Seçili dönemin tamamı: Toplam kontrol ${summary.total} · Yanıt ${summary.reply} · Yanıtsız ${summary.no_reply} · Kontrol hatası ${summary.error} · RTT bulunan başarılı ölçüm ${summary.rtt_count} · Ortalama ${numberText(summary.avg_rtt_ms)} ms · Min ${numberText(summary.min_rtt_ms)} ms · Maks ${numberText(summary.max_rtt_ms)} ms · Yanıt oranı ${summary.response_rate === null ? "—" : "%" + numberText(summary.response_rate)}`;
    const graph = data.graph;
